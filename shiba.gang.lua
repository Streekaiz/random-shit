...
aimbot:Element('Dropdown', 'penetration', {options = {'normal', 'medium', 'high'}})

aimbot:Element('Jumbobox', 'resolver', {options = {'pitch', 'roll', 'arms', 'animation'}})
aimbot:Element('Slider', 'resolver offset', {min = 0, max = 5000, default = 0})
...
players:Element('Slider', 'vcham thickness', {min = -10, max = 0, default = 0})
...
effects:Element("Dropdown", "forcefield type", {options = {"normal", "web", "swirl", "dots"}})
...
world:Element("Dropdown", "hitsound", {options = {"none", "hitmarker", "kombat", "neverlose", "headshot", "headshot 2", "skeet", "rust", "bag", "squash", "lessgo", "MCOOF", "baimware", "osu", "Tf2", "Slap", "moan", "moan 2", "moan 3", "moan4", "elevator", "elevator 2", "1", "Minecraft", "jojo", "vibe", "supersmash", "epic", "retro", "quek"}})
...
world:Element("Toggle", "time changer")

world:Element("Slider", "time", {min = 0, max = 23, default = 10}, function(tbl)
    game:GetService("Lighting").ClockTime = tbl.Slider
end)

game:GetService("Lighting").Changed:Connect(function()
    if values.visuals.world['time changer'].Toggle then
        game:GetService("Lighting").ClockTime = values.visuals.world['time'].Slider
    else 
        oldclocktime = game:GetService("Lighting").ClockTime
    end
end)
...
movement:Element('Slider', 'height amount', {min = -5000, max = 5000, default = 0})

movement:Element('Toggle', 'client offset')
movement:Element('Slider', 'offset (y)', {min = -5000, max = 5000, default = 0})
...
function instancethewatermark()
	...
	title.Text = ' shiba.gang | modified by streekaiz#1132'
...
watermark:Element('TextBox', 'watermark text', {placeholder = 'text here', default = {text = '        shiba.gang | modified by streekaiz#1132'}}, function(tbl)
...
watermark:Element('Slider', 'watermark length', {min = 0, max = 100, default = watermarklocation.watermark.Size.X.Scale}, function(tbl)
...
watermark:Element('Slider', 'watermark offset x', {min = -100, max = 0, default = math.round(watermarklocation.watermark.Position.X.Scale)}, function(tbl)
	watermarklocation.watermark.Position = UDim2.new(0.912 + (tbl.Slider / 150), 0, watermarklocation.watermark.Position.Y.Scale, 0)
end)

watermark:Element('Slider', 'watermark offset y', {min = -100, max = 0, default = math.round(wwatermarklocation.watermark.Position.Y.Scale)}, function(tbl)
					...
					if Player.Team ~= LocalPlayer.Team and Player.Character and Player.Character:FindFirstChild('Humanoid') and not Client.DISABLED and Player.Character:FindFirstChild('Humanoid').Health > 0 and Player.Team ~= 'TTT' and not Player.Character:FindFirstChildOfClass('ForceField') and GetDeg(CamCFrame, Player.Character.Head.Position) <= Stats['max fov'].Slider and Player ~= LocalPlayer then
						if Player.Team ~= game.Players.LocalPlayer.Team and Player:FindFirstChild('Status') and Player.Status.Team.Value ~= game.Players.LocalPlayer.Status.Team.Value and Player.Status.Alive.Value then
							if Client.gun:FindFirstChild('Melee') and values.rage.aimbot['knifebot'].Toggle then
								...
								local Ray = RAY(Origin, (Player.Character.Head.Position - Origin).unit * 10000000) --distance
									...
									if values.rage.aimbot['automatic penetration'].Toggle then
										...
										if EndHit ~= nil and Multipliers[EndHit.Name] ~= nil and #Hits <= 4 then
											if #Hits == 0 then
												...
												if values.rage.aimbot['penetration'].Dropdown == 'high' then 
													penetration = Client.gun.Penetration.Value * 10
												elseif values.rage.aimbot['penetration'].Dropdown == 'medium' then 
													penetration = Client.gun.Penetration.Value * 0.400
												end
