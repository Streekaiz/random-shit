...
do -- Player Metatable
    ...
    function Player_Metatable:Destroy()
        for Index, Component in pairs(self.Components) do
            if tostring(Index) == "Chams" then
                if getgenv().chamsEnabled == true then
                    Component:Destroy()
                end
    ...
    function Player_Metatable:Update()
        ...
        local Chams = getgenv().chamsEnabled == true and self.Components.Chams or true
...
do -- ESP Functions
    function ESP:Player(Instance, Data)
        ...
        Components.Chams = getgenv().chamsEnabled == true and Framework:Instance("Highlight", {Parent = CoreGui, DepthMode = Enum.HighlightDepthMode.AlwaysOnTop}) or true
